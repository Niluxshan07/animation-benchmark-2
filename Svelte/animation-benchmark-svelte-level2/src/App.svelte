<script>
  ...
  const levelBadgeBg = `#2563eb`;
  const levelTagText = `⚙ Level 2: Staggered animations · Cubic-bezier easing · Enhanced hover effects`;
  const levelTagBg = `#eff6ff`;
  const levelTagColor = `#2563eb`;
  const levelTagBorder = `#bfdbfe`;
...
<div class="page">
    ...
    <div class="modal-box" role="dialog" aria-modal="true" on:click|stopPropagation>
      ...
      <div class="modal-body">
        ...
        <div class="modal-item">
          <span class="modal-dot">&#9656;</span><p>{item}</p>
        </div>
        {/each}
      </div>
    </div>
  ...
  <nav class="navbar">
    ...
    <div class="level-badge" style="background:{levelBadgeBg};color:white;padding:6px 18px;border-radius:20px;font-size:0.85rem;font-weight:700;">
      {levelBadgeText}
    </div>
  </nav>
  ...
  <section class="hero">
    <div class="hero-left">
      ...
      <p class="level-tag" style="background:{levelTagBg};color:{levelTagColor};border:1px solid {levelTagBorder};padding:6px 14px;border-radius:20px;font-size:0.83rem;font-weight:600;margin-bottom:18px;">
        {levelTagText}
      </p>
...
<style>
/* ─── Level 2 — Staggered + Cubic-bezier ─────────────────────────────────── */
...
/* ─── Level 2 Keyframe Animations (logo / rings) ──────────────────────────── */
@keyframes dotPulse {
...
/* ─── Level 3 typography (kept for the shadow-depth highlight effect) ───── */
